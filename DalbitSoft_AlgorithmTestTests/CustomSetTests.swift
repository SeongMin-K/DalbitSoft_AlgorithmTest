...
class CustomSetTests: XCTestCase {
    ...
    func testEqual() {
        XCTAssertEqual(CustomSet(contents: [1, 3]), CustomSet(contents: [3, 1]))
        XCTAssertNotEqual(CustomSet(contents: [1, 3]), CustomSet(contents: [3, 1, 5]))
        XCTAssertNotEqual(CustomSet(contents: [1, 3, 5]), CustomSet(contents: [3, 1]))
        XCTAssertNotEqual(CustomSet(contents: [1, 3]), CustomSet(contents: [2, 1]))
    }

    func testNoDuplicates() {
        XCTAssertEqual(CustomSet(contents: [1, 1]), CustomSet(contents: [1]))
    }

    func testDeleteMethod() {
        var expected1 = CustomSet(contents: [3, 2, 1])
        expected1.delete(2)
        XCTAssertEqual(CustomSet(contents: [1, 3]), expected1)

        var expected2 = CustomSet(contents: [3, 2, 1])
        expected2.delete(4)
        XCTAssertEqual(CustomSet(contents: [1, 2, 3]), expected2)
    }

//    func testDifference() {
//        XCTAssertEqual(CustomSet(contents: [1, 3]),
//                       CustomSet(contents: [1, 2, 3]).difference(CustomSet(contents: [2, 4])))
//        XCTAssertEqual(CustomSet(contents: [2, 3]),
//                       CustomSet(contents: [1, 2, 3, 4]).difference(CustomSet(contents: [1, 4])))
//    }

//    func testDisjoint() {
//        XCTAssertTrue(CustomSet(contents: [1, 2]).isDisjoint(CustomSet(contents: [3, 4])))
//        XCTAssertFalse(CustomSet(contents: [1, 2]).isDisjoint(CustomSet(contents: [2, 3])))
//        XCTAssertFalse(CustomSet(contents: [1.0, 2.0]).isDisjoint(CustomSet(contents: [2.0, 3.0])))
//    }
...
//    }
    ...
    func testPutMethod() {
        var expected1 = CustomSet(contents: [1, 2, 4])
        expected1.put(3)
        XCTAssertEqual(CustomSet(contents: [1, 2, 3, 4]),
                       expected1)

        var expected2 = CustomSet(contents: [1, 2, 3])
        expected2.put(3)
        XCTAssertEqual(CustomSet(contents: [1, 2, 3]),
                       expected2)
    }

    func testSize() {
//        XCTAssertEqual(0, CustomSet(contents: emptyTypedArray).size)
        XCTAssertEqual(3, CustomSet(contents: [1, 2, 3]).size)
        XCTAssertEqual(3, CustomSet(contents: [1, 2, 3, 2]).size)
    }
